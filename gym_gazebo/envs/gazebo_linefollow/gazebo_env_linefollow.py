...
class Gazebo_Linefollow_Env(gazebo_env.GazeboEnv):
    ...
    def process_image(self, data):
        ...
        # Reusing code from lab03 to detect middle point 
    
        CANNY_LTHRESH = 50
        CANNY_HTHRESH = 150
        edges = cv2.Canny(cv_image,CANNY_LTHRESH,CANNY_HTHRESH)  
        ...
        state_index = int(center_coord / (int(cols/10)))
        if(center_coord >= 0):

            center = (int(center_coord), rows-FROM_BOTTOM)
            radius = 50  
            red_color = (0, 0, 255)
            cv2.circle(cv_image, center, radius, red_color, thickness=-1)
